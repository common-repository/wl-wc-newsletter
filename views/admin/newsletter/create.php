<div class="wrap">
    <h1>Compose Newsletter | <?php echo WLWCN_NAME ?></h1>
    ...
    <form class="form-newsletter" data-confirmed="0" method="post" action="<?php echo esc_html(admin_url('admin-post.php')) ?>">
        <?php wp_nonce_field('wlwcn_store_newsletter', 'wlwcn_wpnonce'); ?>
        <input type="hidden" name="action" value="wlwcn_store_newsletter">
        <input type="hidden" name="type" value="newsletter">
        <h2>Create email newsletter to send to subscribers here.</h2>
            ...
            <div class="col-12 col-md-8 col-lg-10 col-xl-6">
                ...
                <div class="form-group">
                    <fieldset>
                        ...
                        <input name="subject" type="text" value="<?php echo wlwcn_old('subject') ?>" class="regular-text" required/>
                    </fieldset>
                </div>
                <div class="form-group">
                    ...
                    <p class="help-text">The <b>[Subscriber Name]</b> will be automatically replaced by respective names (full name if available or first or last name if not) of the subscriber, or it will be replaced by <b>there</b> if the subscriber first and last names are not set.</p>
                     <?php
                     $old_msg = wlwcn_old('message') ? wlwcn_old('message') : 'Hello [Subscriber Name],';
                     $settings = [
                ...
                <div class="form-group">
                    <?php
                    $none_check = $selected_list_check = $selected_check = $except_check = $all_check = '';
                    ...
                    <fieldset class="child-1 select2-wlwcn-common wlwcn-mailing-lists <?php echo $mailing_list_select_class ?>">
                        ...
                        <span class="d-block">
                            <select class="select2-wlwcn mailing-list" name="mailing_list">
                                ...
                                <option value="<?php echo $row->id ?>" <?php echo ($row->id == wlwcn_old('mailing_list')) ? 'selected' : '' ?>><?php echo $row->title.' ('.$row->subscribers_count.')' ?></option>
                                <?php } ?>
                            </select>
                        </span>
                    </fieldset>
                    ...
                    <fieldset class="child-1 select2-wlwcn-common wlwcn-subscribers <?php echo $subscriber_select_class ?>">
                        ...
                        <span class="d-block">
                            <select class="select2-wlwcn subscribers" name="subscriber">
                                ...
                                ?>
                                <option value="<?php echo $row->id ?>" <?php echo ($row->id == wlwcn_old('subscriber')) ? 'selected' : '' ?>><?php echo $email_name ?></option>
                                <?php } ?>
                            </select>
                        </span>
                    </fieldset>
